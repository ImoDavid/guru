                  ...
                  const payload = {
                    ...
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("hw"))
                        .map(([key, value]) => [key, value])
                    ),
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("lw"))
                        .map(([key, value]) => [key, value])
                    ),
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("hl"))
                        .map(([key, value]) => [key, value])
                    ),
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("ll"))
                        .map(([key, value]) => [key, value])
                    ),
                    numberOfrows: numb,
                  };
            ...
            >
              {({ isSubmitting }) => (
                <Form>
                  ...
                  <Typography>{`Input your ${numb} values of H for water`}</Typography>
                  {Array.from({ length: numb }).map((_, index) => (
                    <Input
                      key={index}
                      name={`hw${index + 1}`}
                      type="number"
                      placeholder={`H(water)-${index + 1}`}
                      required
                    />
                  ))}
                  <StyledForm>
                    <Typography>{`Input your ${numb} values of L for Water`}</Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
                        name={`lw${index + 1}`}
                        type="number"
                        placeholder={`L(water)-${index + 1}`}
                        required
                      />
                    ))}
                  </StyledForm>
                  <StyledForm>
                    <Typography>{`Input your ${numb} values of H for Liquid`}</Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
                        name={`hl${index + 1}`}
                        type="number"
                        placeholder={`H(liquid)-${index + 1}`}
                        required
                      />
                    ))}
                  </StyledForm>
                  <StyledForm>
                    <Typography>{`Input your ${numb} values of L for Liquid`}</Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
                        name={`ll${index + 1}`}
                        type="number"
                        placeholder={`L(liquid)-${index + 1}`}
                        required
                      />
                    ))}
                  </StyledForm>
