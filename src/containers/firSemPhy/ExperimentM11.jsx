              ...
              onSubmit={(values, { setSubmitting, resetForm }) => {
                setTimeout(async () => {
                  const payload = {
                    ...
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("d1"))
                        .map(([key, value]) => [key, value])
                    ),
                    ...Object.fromEntries(
                      Object.entries(values)
                        .filter(([key]) => key.startsWith("d2"))
                        .map(([key, value]) => [key, value])
                    ),
                    numberOfrows: numb,
                  };
            ...
            >
              {({ isSubmitting }) => (
                <Form>
                  ...
                  <Typography>{`Input your ${numb} values for D`}</Typography>
                  ...
                  <StyledForm>
                    <Typography>
                      {`Input your ${numb} values for `}d<sub>1</sub>{" "}
                    </Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
                  ...
                  <StyledForm>
                    <Typography>
                      {`Input your ${numb} values for `}d<sub>2</sub>
                    </Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
