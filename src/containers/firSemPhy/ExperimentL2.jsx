            ...
            >
              {({ isSubmitting }) => (
                <Form>
                  ...
                  <Typography>{`Input your ${numb} values of BE`}</Typography>
                  {Array.from({ length: numb }).map((_, index) => (
                    <Input
                      key={index}
                      name={`e${index + 1}`}
                      type="number"
                      placeholder={`BE-${index + 1}`}
                      required
                    />
                  ))}
                  <StyledForm>
                    <Typography>{`Input your ${numb} values of BF`}</Typography>
                    {Array.from({ length: numb }).map((_, index) => (
                      <Input
                        key={index}
                        name={`f${index + 1}`}
                        type="number"
                        placeholder={`BF-${index + 1}`}
                        required
                      />
                    ))}
                  </StyledForm>
